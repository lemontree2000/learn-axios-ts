...
describe('helper:headers', () => {
  describe('parseHeaders', () => {
    test('should parse headers', () => {
      ...
      expect(parsed['transfer-encodeing']).toBe('Chunked')
  ...
  describe('processHeaders', () => {
    test('should normalize Content-Type header name', () => {
      const headers: any = {
        'conTent-Type': 'foor/bar',
        'Content-length': 1024
      }
      processHeaders(headers, {})
      expect(headers['Content-Type']).toBe('foor/bar')
  ...
  describe('flattenHeaders', () => {
    ...
    test('should to nothing if headers is undefined or null', () => {
